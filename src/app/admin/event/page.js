...
export default function EventDashboard() {
  ...
  const handleDelete = async (id) => {
    if (!confirm("Hapus event ini? Data peserta juga akan terhapus.")) return;
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '15px' }}>
                    <thead style={{ background: '#f8f8f8', borderBottom: '1px solid #eee' }}>
                        <tr>
                            ...
                            <th style={{ padding: '25px', textAlign: 'center', width: '180px', color:'#444', fontSize:'12px', letterSpacing:'1px' }}>AKSI</th>
                        </tr>
                    </thead>
                    <tbody>
                        {events.map((item) => (
                            <tr key={item.id_event} style={{ borderBottom: '1px solid #f3f4f6' }}>
                                ...
                                <td style={{ textAlign: 'center', padding: '20px' }}>
                                    <div style={{ display: 'flex', justifyContent: 'center', gap: '8px' }}>
                                        ...
                                        <ExportButton eventId={item.id_event} eventName={item.nama_event} />
                                        <button 
                                            onClick={() => handleDelete(item.id_event)} 
                                            style={{ padding:'8px 12px', borderRadius:'8px', border:'none', background:'#fee2e2', cursor:'pointer', color:'#ef4444' }}
                                        >
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
